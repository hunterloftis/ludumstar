...
Game.prototype.replaceState = function(state) {
  ...
  function blendState(remote) {
    ...
    if (local) {
      ...
      // if (local.x[0] && remote.x[0]) {
      //   var dx = remote.x[0] - local.x[0];
      //   remote.x[0] = local.x[0] + dx * blendSpeed;
      // }
      // if (local.y[0] && remote.y[0]) {
      //   var dy = remote.y[0] - local.y[0];
      //   remote.y[0] = local.y[0] + dy * blendSpeed;
      // }
    }
  }
