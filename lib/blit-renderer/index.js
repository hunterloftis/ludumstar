...
function Renderer(container) {
  ...
  this.powerupSprites = {
    ammo: new Blit.Sprite(this.surface, 64, 64)
  };
  ...
  this.powerupSprites.ammo.canvasFrame(0, function drawAmmo(ctx, width, height) {
    ctx.strokeStyle = 'rgba(96, 255, 96, 1)';
    ctx.lineWidth = 10;
...
Renderer.prototype.render = function(seconds, entities, playerId) {
  ...
  function drawPowerup(state) {
    var sprite = powerupSprites[state.powerupType];
    if (!sprite) return;
    surface.push();
